...
import { HotPotItem } from "./menu/HotPotItem";
...
export const Hotpot = ({ match }) => {
  const nameHotpot = match.params.name;

  const foundHotpot = listMenu.find((ele) => ele.name == nameHotpot);

  console.log(foundHotpot);
  return (
    <Fragment>
      <HeaderCommon />
      <HotPotItem
        name={nameHotpot}
        descItem={foundHotpot?.description}
        imgUrl={foundHotpot?.imageUrl}
      />
      <FooterMain />
    </Fragment>
  );
